...
if (
  !cytoscape.layouts ||
  !Object.prototype.hasOwnProperty.call(cytoscape.layouts, "dagre")
) {
  cytoscape.use(dagre);
}
...
if (
  !cytoscape.layouts ||
  !Object.prototype.hasOwnProperty.call(cytoscape.layouts, "fcose")
) {
  cytoscape.use(fcose);
}
    ...
    if (cyRef.current) {
      cyInstanceRef.current = cytoscape({
        ...
        layout: {
          ...
          randomize: false, // Randomize initial positions for better layout
          animate: false,
    ...
    if (cyInstanceRef.current) {
      ...
      if (lastHistoryItem) {
        ...
        const prevNodeIds = new Set(
          lastHistoryItem.graph.nodes.map((n) => n.id)
        );
      ...
      animatingTokens.forEach((token) => {
        ...
        if (sourceNode.length && targetNode.length) {
          ...
          if (path && path.length > 1) {
            ...
            const edgesBySource = graph.edges.filter(
              (e) => e.source === sourceId
            );
            const connectedTransition =
              edgesBySource.length > 0 ? edgesBySource[0].target : null;

            if (connectedTransition) {
              cy.getElementById(`${sourceId}-${connectedTransition}`).addClass(
                "active-path"
              );
              const edgesToTarget = graph.edges.filter(
                (e) => e.source === connectedTransition && e.target === targetId
              );
              if (edgesToTarget.length > 0) {
                cy.getElementById(
                  `${connectedTransition}-${targetId}`
                ).addClass("active-path");
              }
            }
          }
        }
